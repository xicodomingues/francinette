...
void *map_length(void *s)
{
	char *str = malloc(10);
	if (str != NULL)
		sprintf(str, "0x%lX", strlen((char *)s));
	return str;
}
...
int test_single_lstmap(t_list **initial, t_list **expected)
{
	char *list_str = list_to_str(initial);
	set_sign("ft_lstmap(%s, [s => strlen(s)], [x => free(x)])", list_str); free(list_str);
	...
	for (int i = 0; i < malloc_calls; i++)
	{
		...
		if (!no_leaks)
			printf("Most likely you are not calling " BMAG "del" NC \
...
int test_lstmap()
{
	int res = test_single_lstmap(create_list(0), create_list(0));
	res = test_single_lstmap(create_list(1, "hello!"), create_list(1, "0x6"));
	res = test_single_lstmap(
	 	create_list(5, "one", "two", "three", "four", "five"),
	 	create_list(5, "0x3", "0x3", "0x5", "0x4", "0x4"));
	return res;
}
