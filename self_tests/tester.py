...
def compare_to_expected(test_path, test_name, actual):
	...
	console.print("".join(color_diff(diff)))


def execute_test(base_path, project, test_path, test_params, save_output=False):
	...
	if save_output:
		with open(base_path.joinpath(f"self_tests/{test_path}/{test_params[0]}.out"), 'w') as out:
			out.write(actual)
...
def integration_test(base_path):
	for test in TESTS:
		for test_config in test[2]:
			execute_test(base_path, test[0], test[1], test_config, True)